...
class PipelineClient():
    ...
    def publish(self, request_body: dict, pipeline_access_token: str) -> operations.PublishEventResponse:
        ...
        url = utils.generate_url(operations.PublishEventRequest, base_url,
        ...
        query_params = utils.get_query_params(
            operations.PublishEventRequest, request)
        ...
        res = operations.PublishEventResponse(
            status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        ...
        if http_res.status_code == 200:
            ...
            if utils.match_content_type(content_type, 'application/json'):
                ...
                raise errors.ClientError(
                    f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
    ...
    def consume(self, pipeline_access_token: str) -> operations.ConsumeEventResponse:
        ...
        url = utils.generate_url(operations.ConsumeEventRequest, base_url,
                                 '/pipelines/{pipeline_id}/topics/output/events/consume', request)
        headers = utils.get_headers(request)
        query_params = utils.get_query_params(
            operations.ConsumeEventRequest, request)
        headers['Accept'] = 'application/json'
        headers['user-agent'] = self.glassflow_client.glassflow_config.user_agent
        client = self.glassflow_client.glassflow_config.client
        http_res = client.request(
